...
LEGACY_JSON = ROOT / "legacy-library-data.json"
OUTPUT_JSON = ROOT / "music-library-data.json"
CACHE_JSON = ROOT / "library-cache.json"  # 旧版互換。SQLite版では読み書きしません。
DIAG_JSON = ROOT / "library-diagnostics.json"
DIAG_CSV = ROOT / "library-diagnostics.csv"
MP3_INDEX_JSON = ROOT / "mp3-index.json"
ARTWORK_INDEX_JSON = ROOT / "artwork-index.json"
ARTWORK_CACHE = ROOT / ".artwork-cache"
...
def safe_rel(path: Path) -> str:
    """Return a UTF-8-safe path relative to the package root, using '/' separators."""
...
def main() -> int:
    ...
    if backup_path:
        print(f"SQLite backup          : {backup_path.relative_to(ROOT)}")
            ...
            try:
                ...
                if metadata_unchanged and existing_row is not None:
                    ...
                    if cached_source == "embedded":
                        artwork_unchanged = bool(cached_artwork) and (ROOT / cached_artwork).is_file()
