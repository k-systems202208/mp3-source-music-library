...
<!-- BEGIN WINDOWS-INSTALLER-V2.7.3 -->
...
v2.7.3では、TailscaleのHTTPS接続先をスマートフォンのホーム画面へ追加し、専用アイコンからアプリのように起動できるようになりました。

- iPhone・iPad・Android向けアプリアイコン
- アプリ名「自宅音楽ライブラリ」、短縮名「音楽ライブラリ」
- ホーム画面からのstandalone起動
- 起動中画面と通信不能時の案内
- 利用者メニューの「スマホのホーム画面に追加」案内
- 現在の接続先URL表示・コピー
- ローカル接続、Tailscale HTTPS、通常HTTPの区別
- safe-area、44px以上のタップ領域、縦横表示対応
- 選択中スキンに応じたテーマ色
- 表示資産だけを対象とする安全なService Workerキャッシュ

MP3、API応答、SQLite DB、バックアップ、アートワーク、認証情報は端末キャッシュへ保存しません。オフライン音楽再生機能ではなく、Wi-FiまたはTailscale接続が必要です。

### ダウンロードと更新

1. [v2.7.3 Release](https://github.com/k-systems202208/mp3-source-music-library/releases/tag/v2.7.3)を開く
2. `MusicLibrary-Setup-2.7.3-x64.exe`をダウンロード
3. 自宅音楽ライブラリを終了
4. インストーラーを実行
5. 「自宅音楽ライブラリ」を起動

v2.7.2をアンインストールせず、そのまま上書き更新できます。既存のMP3参照先、収録曲、利用者、再生回数、お気に入り、スキン、設定、バックアップは維持されます。

### スマートフォンのホーム画面へ追加

外出先でも利用する場合は、普段使用するTailscale HTTPS URLをSafariまたはChromeで開いてからホーム画面へ追加します。

```text
https://PC名.tailnet名.ts.net/music-library-search.html
```

iPhone・iPadではSafariの共有ボタンから「ホーム画面に追加」、Androidではブラウザメニューから「アプリをインストール」または「ホーム画面に追加」を選びます。
