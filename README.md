...
> 最新公開版: `v2.7.7`（2026-08-04公開）
> ドキュメント監査日: 2026-08-04

## v2.7.7の主な変更

- プレイリストの複製と、PCでのドラッグ並び替え
- オーナー限定のアルバム表示名変更（MP3タグは変更しない）
- 家族ユーザーを含む、本人による自分の表示名変更
- オーナー限定の管理・診断画面と診断JSON保存
- DBスキーマ7を継続し、既存データをそのまま利用

実装内容と確認方法は[v2.7.7実装・確認記録](docs/21-next-update-implementation.md)を参照してください。
...
| v2.7.6 | メタデータ編集のオーナー限定化、プレーヤーのアーティスト・アルバムリンク |
| v2.7.7 | プレイリスト操作改善、アルバム表示名、本人表示名、管理・診断画面 |

## 実機確認済みの構成

開発・受入環境では、次の状態でv2.7.7を確認しました。これは製品の固定上限ではなく、確認に使用したライブラリの実績値です。
...
- v2.7.7の全ソース回帰試験、EXEスモーク試験、Inno Setupコンパイルに合格
...
1. GitHub Releasesから`MusicLibrary-Setup-2.7.7-x64.exe`を取得します。
...
- [v2.7.7実装・確認記録](docs/21-next-update-implementation.md)
- [v2.7.7リリース監査結果](docs/22-release-audit-v2.7.7.md)

Windows配布物に同梱する文書は[インストーラー文書一覧](windows-installer/docs/DOCUMENT_INDEX_v2.7.7.md)を参照してください。
