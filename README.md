...
<!-- BEGIN WINDOWS-INSTALLER-V2.7.4 -->
...
v2.7.4では、Windowsで260文字以上になる深いフォルダーや長いファイル名のMP3を、ファイル名を変更せずにスキャン・登録・再生できるようになりました。

- ローカルドライブの長いWindowsパスに対応
- UNCネットワークパスの長いパス形式に対応
- MP3走査、ファイル情報、ID3タグ、音声情報を長いパス対応
- 埋め込み・外部アートワーク取得を長いパス対応
- 移動検出用の内容署名を長いパス対応
- MP3配信とHTTP Rangeシークを長いパス対応
- SQLiteには従来どおり通常の相対パスを保存
- 起動直後のローカルオーナー認証画面が停止する現象を修正

実機では、従来読み込めなかった201曲を含む8,480曲をエラー0件で登録し、最長364文字の曲をPCブラウザで再生・シークできることを確認しました。

### ダウンロードと更新

1. [v2.7.4 Release](https://github.com/k-systems202208/mp3-source-music-library/releases/tag/v2.7.4)を開く
2. `MusicLibrary-Setup-2.7.4-x64.exe`をダウンロード
3. 自宅音楽ライブラリを終了
4. インストーラーを実行
5. 「自宅音楽ライブラリ」を起動

v2.7.3をアンインストールせず、そのまま上書き更新できます。既存のMP3参照先、利用者、再生回数、お気に入り、スキン、設定、バックアップは維持されます。

### 長いファイルパス

Windows内部でファイルへアクセスするときだけ長いパス形式を使用します。MP3、タグ、ファイル名、フォルダー名は変更しません。DBには音楽フォルダーからの通常の相対パスを保存します。
...
- スマートフォンのホーム画面追加は音楽のオフライン保存を行いません
