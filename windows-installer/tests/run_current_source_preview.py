...
def main() -> int:
    ...
    try:
        ...
        print()
        print("現行ソースの確認画面を2つ開きます。")
        print("  1. オーナー画面: 表示名、メタデータ編集、管理・診断あり")
        print("  2. 匿名画面: 利用者専用機能と編集ボタンなし")
        print()
        print("確認項目:")
        print("  - 『訂正済のみ』ボタンと『訂正済』表示がない")
        print("  - オーナー画面だけ編集ボタンが表示される")
        print("  - アルバムカードからアルバム表示名を変更できる")
        print("  - プレイヤーのアーティスト名・アルバム名から一覧へ移動できる")
        print("  - プレイリストを複製し、ドラッグまたは上下ボタンで並べ替えられる")
        print("  - 利用者画面で自分の表示名と管理・診断を確認できる")
        print("  - 家族ユーザーの表示名変更は実際のTailscale接続で確認する")
        print()
