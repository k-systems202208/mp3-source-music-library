...
assert 'CURRENT_VERSION = "2.7.4"' in update_text
assert "MusicLibrary-UpdateChecker/2.7.4" in update_text
assert "/releases?per_page=100" in update_text
assert "/releases/latest" not in update_text

# Schema 6 remains current. v2.7.4 must not alter or bypass the v2.7.2 migration backup.
...
assert '#define MyAppVersion "2.7.4"' in installer_text
...
assert "filevers=(2, 7, 4, 0)" in version_info
assert "prodvers=(2, 7, 4, 0)" in version_info
assert "FileVersion', u'2.7.4'" in version_info
assert "ProductVersion', u'2.7.4'" in version_info
...
assert "music-library-shell-v2.7.4" in worker_text
...
assert "def open_path(" in long_paths_text
...
assert "HTTPStatus.SEE_OTHER" not in server_text[server_text.index("def handle_local_owner_exchange"):server_text.index("def handle_current_user")]
...
required_tests = [
    ...
    "test_pwa_server.py", "test_long_path_support.py", "test_live_long_path_probe.py",
    "test_phase2_copied_full_scan_workflow.py", "test_release_candidate.py",
]
for test_name in required_tests:
    assert test_name in build_script, test_name
assert "MusicLibrary-Setup-2.7.4-x64.exe" in build_script
assert "v2.7.4 RC2" in build_script
assert "BUILD_REPORT_v2.7.4_RC2.txt" in build_script

required_files = [
    ROOT / "RELEASE_NOTES_v2.7.4.md",
    ...
    ROOT / "docs" / "MANUAL_TEST_v2.7.4.txt",
    ROOT / "docs" / "GITHUB_RELEASE_2.7.4.txt",
    ROOT / "docs" / "DOCUMENT_VERSION_CHECK_v2.7.4.txt",
    ROOT / "docs" / "RC1_v2.7.4_SCOPE.md",
    ROOT / "docs" / "RC2_v2.7.4_SCOPE.md",
    ROOT / "docs" / "LONG_PATH_v2.7.4_PHASE1.md",
    ROOT / "docs" / "LONG_PATH_v2.7.4_PHASE2.md",
]
...
assert "[string]$Version = '2.7.4'" in release_asset_script
