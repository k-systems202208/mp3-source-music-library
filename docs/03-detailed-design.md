...
> 本文書はモジュール単位の概要です。別のプログラマーが同じアプリを再実装するための完全な実装契約は、[自宅音楽ライブラリ v2.7.7 再実装用詳細設計書](23-reimplementation-detailed-design.md)を参照してください。

## `paths.py`

アプリ名、バージョン、データ保存先、DB、ログ、バックアップ、静的資産のパスを一元管理します。現行`APP_VERSION`は`2.7.7`です。
...
- 曲レコードとMP3は保持
- 複製は新しいplaylist IDを作り、元の曲順をコピー
- ドラッグと上／下ボタンは同じ曲順更新APIを使用
...
- DB・設定・ソースなど禁止資産を拒否
- 本人表示名の更新対象を現在の利用者IDに固定
- アルバム表示名の更新をオーナーだけに許可
- 管理診断APIをオーナーだけに許可
...
`CURRENT_VERSION = 2.7.7`としてGitHub Releases APIを確認します。公開済みReleaseとプレリリースを比較し、現在版より新しい場合だけ通知します。通信失敗はライブラリ機能へ影響させません。

## `music-library-search.html`

単一ページUIとしてホーム、曲名、アーティスト、アルバム、プレイリスト、プレーヤー、本人表示名、利用者管理、管理診断、スキン、バックアップ、新版通知、PWA案内を提供します。

PCのプレイリスト曲順はHTML Drag and Dropで操作し、保存には既存の`tracks/order` APIを使用します。スマートフォンとキーボード操作向けに上／下ボタンを残します。
