...
2. `MusicLibrary-Setup-2.7.7-x64.exe`を実行します。
...
右上の利用者ボタンから、自分の表示名、利用者情報、スキン、バックアップ、新版通知、スマートフォン追加案内を開きます。オーナーには管理・診断情報も表示されます。

### 自分の表示名を変更する

認証済みの利用者は、オーナーでも家族ユーザーでも自分の表示名を変更できます。

1. 右上の利用者ボタンを押す
2. 「自分の表示名」の入力欄へ新しい名前を入力する
3. 「表示名を保存」を押す

変更できるのは自分の表示名だけです。空の名前は保存できません。家族ユーザーが変更した表示名は、その後Tailscaleから取得するアカウント名が変わっても保持されます。
...
- 1曲リピート
- 音量
- アーティスト名を押して、そのアーティストのアルバム一覧へ移動
- アルバム名を押して、そのアルバムの曲一覧へ移動

MP3は再エンコードしません。

曲名、アーティスト名、アルバム名の表示変更はオーナーだけが行えます。オーナーで開くと対象の横に編集ボタンが表示され、家族ユーザーや匿名接続には表示されません。アルバム名は「アルバム」タブのアルバムカードから変更します。同じアルバムに属する曲へ一括反映されます。

変更内容はライブラリDBの表示情報へ保存され、MP3ファイル自体や埋め込みタグは編集しません。元の表示へ戻す場合は、編集画面で補正値を空にして保存します。
...
- 「複製」で名前と曲順を含むコピーを作成
- 名前変更
- PCでは曲番号のハンドルをドラッグして曲順を変更
- スマートフォンでは上／下ボタンで曲順を変更
- 曲を外す
- プレイリスト削除

複製後の名前は「元の名前 のコピー」です。同名がある場合は連番が付きます。複製したプレイリストは元とは別に編集できます。

曲を外したりプレイリストを削除しても、MP3と通常の曲一覧は消えません。他の利用者のプレイリストは表示・変更できません。
...
オーナーは利用者ボタンの「管理・診断」で次を確認できます。

- DBの整合性とスキーマ番号
- 登録利用者数とプレイリスト数
- 最新スキャンと問題件数
- DB、音楽、バックアップ、ログの保存先
- バックアップ件数、容量、最新作成日時、復元予約

「再読み込み」で最新状態へ更新できます。「診断JSONを保存」でサポートへ渡せる診断ファイルをダウンロードできます。診断JSONには保存先などの環境情報が含まれるため、公開場所へそのまま掲載しないでください。家族ユーザーや匿名接続には診断欄を表示せず、APIへの直接アクセスも拒否します。
