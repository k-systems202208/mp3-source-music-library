...
2. `MusicLibrary-Setup-2.7.6-x64.exe`を実行します。
...
曲名とアーティスト名の表示変更はオーナーだけが行えます。オーナーで開くと曲名またはアーティスト名の横に編集ボタンが表示され、家族ユーザーや匿名接続には表示されません。変更内容はライブラリDBの表示情報へ保存され、MP3ファイル自体は編集しません。
