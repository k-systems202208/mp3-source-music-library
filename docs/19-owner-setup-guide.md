...
**対象:** Windows 10／11 64bit・自宅音楽ライブラリ v2.7.5  
**文書版:** 1.0（2026-08-03）  
**読者:** PCのインストールやネットワーク設定に慣れていないオーナー

印刷・配布用: [Word版オーナーガイド](downloads/music-library-owner-setup-guide-v2.7.5.docx)
...
<https://github.com/k-systems202208/mp3-source-music-library/releases/tag/v2.7.5>

ページ上部で次を確認します。

- タイトルが「自宅音楽ライブラリ v2.7.5」
...
1. `MusicLibrary-Setup-2.7.5-x64.exe`
2. `SHA256SUMS.txt`
...
```powershell
Get-FileHash -Algorithm SHA256 "$env:USERPROFILE\Downloads\MusicLibrary-Setup-2.7.5-x64.exe"
```
...
1. `MusicLibrary-Setup-2.7.5-x64.exe` をダブルクリックします。
...
**完了の目印:** 「自宅音楽ライブラリ 2.7.5」の管理画面が開きます。
...
本書の操作ラベルは自宅音楽ライブラリ v2.7.5の実装、同梱文書、公開情報に合わせています。図とGIFは個人情報を含まない再現画面です。Tailscaleの画面・用語は公式サービスの更新によって変わる場合があるため、迷った場合は上記の公式リンクも確認してください。
